...
FILE * CreateFile(long size1,long size2,char *name,char *data1,char *data2)
{
    ...
    char *finalData = malloc(finalSize+1);
    memcpy(finalData,data1,size1);
    memcpy(finalData+size1,data2,size2);
    finalData[finalSize]=0;
    fprintf(stdout,"%s",finalData);
    FILE *newFile = fopen(name,"w");
    if(!newFile){
        perror("unable to create file");
    }
    fseek(newFile,0,SEEK_SET);
    fputs(finalData,newFile);
    //fclose(newFile);
    free(finalData);
    return newFile;
}
